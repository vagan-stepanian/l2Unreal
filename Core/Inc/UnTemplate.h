...
template <class T> class TRoughArray : public TArray<T>, public FLazyLoader
{
public:
	TRoughArray(INT InNum = 0)
		: TArray<T>(InNum)
		, FLazyLoader()
	{}
	~TRoughArray()
	...
	friend FArchive& operator<<(FArchive& Ar, TRoughArray& This)
	{
		guard(TRoughArray << );
		This.CountBytes(Ar);
		if (Ar.IsLoading()) {
			INT StartPosition = 0;
			if (Ar.Ver() >= 123 && Ar.LicenseeVer() >= 25) {
				Ar << StartPosition;
			}
			INT NewNum = 0; // gam
			Ar << AR_INDEX(NewNum);
			...
			for (INT i = 0; i < NewNum; i++) {
				...
				if (Ar.Ver() >= 123 && Ar.LicenseeVer() >= 25) {
					Ar << NextElementPosition;
				}
				Ar << *new(This)T;
				Ar.Seek(NextElementPosition);
			}

			Ar.Seek(StartPosition);
		}
		else if (Ar.IsSaving()) {
			INT StartPosition = Ar.Tell();
			if (Ar.Ver() >= 123 && Ar.LicenseeVer() >= 25) {
				Ar << StartPosition;
			}
			Ar << AR_INDEX(This.ArrayNum);
			for (INT i = 0; i < This.ArrayNum; ++i) {
				INT ElementStartPosition = Ar.Tell();
				if (Ar.Ver() >= 123 && Ar.LicenseeVer() >= 25) {
					Ar << ElementStartPosition;
				}

				Ar << This(i);

				if (Ar.Ver() >= 123 && Ar.LicenseeVer() >= 25) {
					INT CurrentPosition = Ar.Tell();
					Ar.Seek(ElementStartPosition);
					Ar << CurrentPosition;
					Ar.Seek(CurrentPosition);
				}
			}
			INT EndPositionOfArray = Ar.Tell();
			Ar.Seek(StartPosition);
			Ar << EndPositionOfArray;
			Ar.Seek(EndPositionOfArray);
		}
		return Ar;
		unguard;
	}
